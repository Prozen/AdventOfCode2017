val dat = """mypyemv (1058) -> tdssotr, pebnvks, zaulju
snslv (48)
aytbgf (30)
mkcrsy (42)
shwcd (42)
...
fun main(args: Array<String>) {
    val splitted = dat.filter { it.contains("->") }
            .map { it.split("->") }
            .map {
                Pair(it[0].split(" ")[0], it[1].split(",")
                        .map { it.trim() }
                )
            }
    print(splitted.filter { pair ->
        !splitted.flatMap { it.second }.contains(pair.first)
    })
}